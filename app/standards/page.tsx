...
export default function StandardsPage() {
  ...
  const handleUpdateVotingTime = async (standard: Standard) => {
    const hours = parseInt(prompt("How many hours to extend from now?", "1") || "0")
    const minutes = parseInt(prompt("And how many minutes?", "0") || "0")
  ...
  const handleEditVotes = async (standard: Standard) => {
    if (!standard.votes) return

    const approveCount = standard.votes.filter(v => v.vote_type === "approve").length
    const denyCount = standard.votes.filter(v => v.vote_type === "deny").length

    const newApprove = parseInt(prompt(`Current Approve votes: ${approveCount}\nEnter new Approve count:`, `${approveCount}`) || "")
    const newDeny = parseInt(prompt(`Current Deny votes: ${denyCount}\nEnter new Deny count:`, `${denyCount}`) || "")

    if (isNaN(newApprove) || isNaN(newDeny) || newApprove < 0 || newDeny < 0) {
      alert("Invalid vote counts.")
      return
    }

    // Delete existing votes for this standard
    await supabase.from("votes").delete().eq("standard_id", standard.id)

    // Insert new votes based on counts (user_id is dummy here)
    const newVotes = []
    for (let i = 0; i < newApprove; i++) {
      newVotes.push({ standard_id: standard.id, vote_type: "approve", user_id: `admin-auto-${i}` })
    }
    for (let i = 0; i < newDeny; i++) {
      newVotes.push({ standard_id: standard.id, vote_type: "deny", user_id: `admin-auto-${i + newApprove}` })
    }
              ...
              <CardContent>
                ...
                {isAdmin && (
                  <div className="space-x-2 mb-3">
                    <Button size="sm" onClick={() => handleSubVote(standard.id, "add", prompt("Add content:") || "")}>
                      <Plus className="w-3 h-3 mr-1" /> Add
                    </Button>
                    <Button size="sm" onClick={() => handleSubVote(standard.id, "edit", prompt("Edit content:") || "")}>
                      <Pencil className="w-3 h-3 mr-1" /> Edit
                    </Button>
                    <Button
                      size="sm"
                      variant="destructive"
                      onClick={() => handleSubVote(standard.id, "remove", prompt("Remove what?") || "")}
                    >
                      <Trash2 className="w-3 h-3 mr-1" /> Remove
                    </Button>
                    <Button size="sm" onClick={() => handleAddToDoc(standard)}>
                      📎 Add to Doc
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => handleUpdateVotingTime(standard)}>
                      ⏱ Change Vote Time
                    </Button>
                    <Button size="sm" variant="outline" onClick={() => handleEditVotes(standard)}>
                      ✏️ Edit Votes
                    </Button>
                  </div>
                )}
                ...
                <div className="mt-4 space-y-2">
                  {subVotes
                    .filter((sv) => sv.standard_id === standard.id)
                    .map((sv) => (
